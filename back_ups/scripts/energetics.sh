...
runcalc(){
   ...
   gmx grompp -f $MDP/minimize.mdp -po min.mdp.out -c $MOLEC.gro -p $MOLEC.top -o minimize.tpr
   
   
   check minimize.tpr 
   gmx mdrun -nt 1 -s minimize.tpr -c $MOLEC.minimized.gro -e ener.min.edr -g md.min.log -o traj.min.trr

   check $MOLEC.minimized.gro 
   gmx grompp -f $MDP/vac_md.mdp -po vac_md.mdp.out -c $MOLEC.minimized.gro -p $MOLEC.top -o vac_md.tpr 

   check vac_md.tpr
   gmx mdrun -nt 1 -s vac_md.tpr -c $MOLEC.vac_md_end.gro -e ener.vac_md.edr -g md.vac_md.log -o trac.vac_md.trr 
}
